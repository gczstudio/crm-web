  ...
  data() {
    return {
      ...
      globalRules: {
        ...
        requiredOrderNumber: [{ required: true, message: "字段不能为空" }, { validator: validator.orderNumber }],
        input: [
          { validator: validator.speChar, message: "输入信息包含特殊字符" },
          { max: 500, message: "输入信息不能超过500个字", trigger: "blur" },
        ],
        input500: [{ max: 500, message: "输入信息不能超过500个字", trigger: "blur" }],
        desc: [{ max: 100, message: "输入信息不能超过100个字", trigger: "blur" }],
      },
    };
  },
  computed: {
