...
service.interceptors.response.use(
  ...
  (error) => {
    if(error.response && error.response.status === 401) {
      if(!isRefreshing) {
        isRefreshing = true;
        var tokenInfo = JSON.parse(sessionStorage.getItem('tokenInfo') as any);
        const headers = {
        ...
        return UserModule.refreshLogin({
          ...
          return getSessionInfo().then(res=> {
            ...
            var checkedRole = getCheckedRole(res);
            var currentRole = sessionStorage.getItem('currentRole') ? JSON.parse((sessionStorage.getItem('currentRole')) as any) : '';
