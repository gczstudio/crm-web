...
export const getCheckedRole = (data?: any) => {
  var roles = (data || JSON.parse((sessionStorage.getItem('userInfo') as string)))?.userRoles || [];
...
export const toFixed = (num: any, s = 2) => {
  var result = Math.round(num*Math.pow(10, s)) / Math.pow(10, s) + ''
  var pos = result.split('.')[1]?.length || 0
  if(result.indexOf('.') === -1 && s) {
    result += '.';
  }
  for (var i=0;i< s - pos; i++ ) {
    result += '0';
  }
  return result;
}
...
export const getRouteByMenuId = (id:string) => {
  var menus = JSON.parse((sessionStorage.getItem('contrMgr') as string))?.menu || [];
...
export const downFile = (opts:any) => {
  ...
  export const exportTable =  (config: any) => {
    ...
    if (typeof ref === 'number') {
      ...
      var refs = _this.$refs[ref || 'refTable'];
  ...
  export const formatQueryField = (paramArrs: any) => {
    ...
    return paramArrs.filter( (item: any) => {
      return item;
    }).join(',');
  };
  ...
  export const formPost = (options: any) => {
    ...
    form.method = 'post';
    form.target = '_blank';
    for (var key in options.loginInfo) {
      var myInput = document.createElement('input');
      myInput.setAttribute('name', key);
