...
class User extends VuexModule implements IUserState {
  ...
  public async Login(payload: any) {
    const res: any = await login(qs.stringify(payload.data), { headers: payload.headers });
  ...
  public async GetUserInfo(flag?: boolean) {
    const res: any = await getSessionInfo();
  ...
  public async GetMenus(route?: any) {
    const res: any = await getMenuAndContr();
    ...
    const realPath = route?.path || PermissionModule.asyncRoutes[0].path;
    // 切换角色时，重定向一下
    if (this.isRenderTag) {
      router
        .replace({
  ...
  public Reset() {
    ...
    resetRouter();
    // 不清除authType
    const authType: any = localStorage.getItem("authType");
    localStorage.clear();
