    ...
    <svg xmlns="http://www.w3.org/2000/svg" version="1.1" style="width: 100%; height: 100%">
      <template v-for="item in graphData">
        ...
        <g v-if="item.type === 'square'" id="square" :key="item.id" @mousedown.stop="moveMousedown(item, $event)">
          <rect :width="item.width" :height="item.height" :x="item.x" :y="item.y" style="fill: rgb(0, 0, 255); stroke-width: 1; stroke: rgb(0, 0, 0); cursor: move" />
          <circle v-if="item.active" :cx="item.x1" :cy="item.y1" r="5" stroke="black" stroke-width="1" fill="#fff" style="cursor: nw-resize" @mousedown.stop="sizeMousedown(item, 'start', $event)" />
          <circle v-if="item.active" :cx="item.x2" :cy="item.y2" r="5" stroke="black" stroke-width="1" fill="#fff" style="cursor: ne-resize" @mousedown.stop="sizeMousedown(item, 'end', $event)" />
        </g>
        <!-- 圆角矩形 -->
        <g v-if="item.type === 'roundSquare'" id="roundSquare" :key="item.id" @mousedown.stop="moveMousedown(item, $event)">
          <rect :width="item.width" :height="item.height" :x="item.x" :y="item.y" rx="10" ry="10" style="fill: rgb(0, 0, 255); stroke-width: 1; stroke: rgb(0, 0, 0); cursor: move" />
          <circle v-if="item.active" :cx="item.x1" :cy="item.y1" r="5" stroke="black" stroke-width="1" fill="#fff" style="cursor: nw-resize" @mousedown.stop="sizeMousedown(item, 'start', $event)" />
          <circle v-if="item.active" :cx="item.x2" :cy="item.y2" r="5" stroke="black" stroke-width="1" fill="#fff" style="cursor: ne-resize" @mousedown.stop="sizeMousedown(item, 'end', $event)" />
        </g>
        <!-- 半圆角矩形 -->
        <g v-if="item.type === 'halfRoundSquare'" id="halfRoundSquare" :key="item.id" @mousedown.stop="moveMousedown(item, $event)">
          <rect
            :width="item.width"
            :height="item.height"
            :x="item.x"
        ...
        <!-- 圆形 -->
        <g v-if="item.type === 'circle'" id="circle" :key="item.id" @mousedown.stop="moveMousedown(item, $event)">
          <circle :cx="(item.x1 + item.x2) / 2" :cy="(item.y1 + item.y2) / 2" :r="item.height / 2" stroke="black" stroke-width="2" fill="red" style="cursor: move" />
          <circle v-if="item.active" :cx="item.x1" :cy="item.y1" r="5" stroke="black" stroke-width="1" fill="#fff" style="cursor: nw-resize" @mousedown.stop="sizeMousedown(item, 'start', $event)" />
          <circle v-if="item.active" :cx="item.x2" :cy="item.y2" r="5" stroke="black" stroke-width="1" fill="#fff" style="cursor: ne-resize" @mousedown.stop="sizeMousedown(item, 'end', $event)" />
        </g>

        <!-- 箭头 -->
        <g v-if="item.type === 'rightArrow' || item.type === 'leftArrow' || item.type === 'roundSquareLabel'" id="arrow" :key="item.id" @mousedown.stop="moveMousedown(item, $event)">
          <path :d="item.path" style="stroke: none; fill: #000000; cursor: move"></path>
          <circle v-if="item.active" :cx="item.x1" :cy="item.y1" r="5" stroke="black" stroke-width="1" fill="#fff" style="cursor: nw-resize" @mousedown.stop="sizeMousedown(item, 'start', $event)" />
          <circle v-if="item.active" :cx="item.x2" :cy="item.y2" r="5" stroke="black" stroke-width="1" fill="#fff" style="cursor: ne-resize" @mousedown.stop="sizeMousedown(item, 'end', $event)" />
        </g>
        <!-- 文本输入框 -->
        <g v-if="item.type === 'text'" id="text" :key="item.id" @mousedown.stop="moveMousedown(item, $event)">
