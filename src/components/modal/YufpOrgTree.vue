<template>
  <el-popover v-model="visiable" :width="width" :visible-arrow="false" popper-class="org-popover" :trigger="disabled ? 'manual': 'click'">
      ...
      <el-input slot="reference" ref="orgTreeInput" v-model="searchVal" :readonly="options.readonly" :disabled="disabled" :placeholder="placeholder" suffix-icon="el-icon-arrow-down" @clear="clearFn" clearable></el-input>
  </el-popover>
</template>
...
export default {
  ...
  methods: {
    ...
    getLabelString(ids) {
      return this.originTreeData.filter(item =>ids.includes(item[this.options.dataId])).map(item => item[this.options.dataLabel]).join(',')
    },
