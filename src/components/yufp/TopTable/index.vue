<template>
  <div :class="{ 'yufp-top10-table': true, border: border }">
    ...
    <div ref="table" class="top10-table__table" v-mloading="loading">
      <el-table ref="table" :key="JSON.stringify(tableData)" :data="tableData" :height="height" border style="width: 100%">
        ...
        <el-table-column v-for="(column, index) in columns" :key="index" v-bind="column">
          <template slot-scope="scope">
            <div
              @click.prevent="column.onClick ? column.onClick(scope) : null"
              :class="{ 'render-box': true, num: column.isNum || column.formatMoney }"
              v-html="column.render ? column.render(scope) : column.formatMoney ? $util.formatMoney(scope.row[column.prop]) : scope.row[column.prop]"
            ></div>
          </template>
        </el-table-column>
        <div slot="empty">
...
<style lang="scss">
.yufp-top10-table .el-table th > .cell {
  padding: 0 5px;
  white-space: nowrap;
  background: #f2f2f2;
}
.yufp-top10-table .top10-table__header {
  position: relative;
  margin-bottom: 16px;
}
.yufp-top10-table .view-more {
  top: 5px;
  right: 0;
  z-index: 10;
}
.yufp-top10-table .top10-table__title {
  color: #333;
  font-size: 16px;
  font-weight: bold;
...
.yufp-top10-table .top10-table__title .icon-cunkuanyue {
  color: #47a1fb;
  margin-right: 8px;
}
.yufp-top10-table .top10-table__table .el-table th {
  color: #333;
  font-weight: bold;
  background: #f2f2f2;
  height: 46px;
...
.top10-table__table .el-table td {
  padding: 0;
}
.top10-table__table .el-table td .cell {
  padding: 0 5px;
  color: #333;
  height: 40px;
  line-height: 40px;
}
.top10-table__table .el-table td .el-tooltip .render-box {
  display: inline;
}
.top10-table__table .el-table td .cell .up {
  color: #f14922;
}
.top10-table__table .el-table td .cell .down {
  color: #0fcb51;
}
.yufp-top10-table .el-table--border td,
.yufp-top10-table .el-table--border th {
  border-color: #d8d8d8;
}
.yufp-top10-table .el-table--border,
.yufp-top10-table .el-table--group {
  border-color: #d8d8d8;
}
.yufp-top10-table .el-table td,
.yufp-top10-table .el-table th.is-leaf {
  border-color: #d8d8d8;
}
.yufp-top10-table .el-table__fixed-right::before,
.yufp-top10-table .el-table__fixed::before {
  background-color: #d8d8d8;
}
.yufp-top10-table .el-table--border::after,
.yufp-top10-table .el-table--group::after,
.yufp-top10-table .el-table::before {
  background-color: #d8d8d8;
}
.yufp-top10-table .el-table--border td:first-child .cell,
.yufp-top10-table .el-table--border th:first-child .cell {
  padding-left: 5px;
}

.yufp-top10-table .rank-cell {
  position: relative;
  height: 100%;
}
.yufp-top10-table .rank-cell .icon-rank {
  position: absolute;
  top: 50%;
  left: 50%;
  transform: translate(-50%, -50%);
}
.yufp-top10-table .rank-cell .rank-no {
