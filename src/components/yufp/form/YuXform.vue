    ...
    methods: {
      ...
      clearValidate: function (props) {
        ...
        var fields = props.length
          ? (typeof props === 'string'
            ? this.fields.filter( function (field) { return props === field.name; })
            : this.fields.filter( function (field) { return props.indexOf(field.name) > -1; })
          ) : this.fields;
      ...
      validateField: function (props, cb) {
        props = [].concat(props);
        var fields = this.fields.filter(function (field) { return props.indexOf(field.name) !== -1;});
        if (!fields.length) {
...
<style lang="scss">
.yu-xform { position: relative; background: #fff;}
.yu-xform.expand { box-shadow: 0px 3px 6px rgba(0, 0, 0, 0.2);}
.yu-xform .yu-xform-content { width: 100%; }
.yu-xform .yu-xform-content--search { width: calc(100% - 200px); }
.yu-xform .yu-xform-content--details { width: 100%; }
.yu-xform .yu-xform-content--expand { width: calc(100% - 225px); }
.yu-xform .yu-xform-btns { text-align: center; position: absolute; right: 16px;bottom: 16px; }
.yu-xform  .icon-btn {font-weight: bold;font-size: 15px;position: relative;top: 1px;}
.yu-xform .arrow-down { position: relative; top: 15px; margin-left: 8px; width: 0; height: 0; border-left: 6px solid transparent;border-right: 6px solid transparent; 
// border-top: 8px solid $blue; 
border-radius: 2px; }
.yu-xform .arrow-up { position: relative; bottom: 13px; margin-left: 8px; width: 0; height: 0; border-left: 6px solid transparent;border-right: 6px solid transparent; 
// border-bottom: 8px solid $blue;
border-radius: 2px; }
// form表单表格化
.yu-xform-table {margin-bottom: 15px; position: relative;}
.yu-xform-table::before { content: ''; display: inline-block; position: absolute; top: 0; left: 0; z-index: 1; width: 100%; height: 1px; background: #E1E1E1;}
.yu-xform-table::after { content: ''; display: inline-block; position: absolute; top: 0; left: 0; z-index: 1; width: 1px; height: 100%; background: #E1E1E1;}
.yu-xform-table .el-input.is-disabled .el-input__inner {background-color: #FFF;border: none;}
.yu-xform-table .el-form-item {margin-bottom: 0px; background: #E4F2F6;}
.yu-xform-table .el-form-item__content { border-left: 1px solid #E1E1E1; background: #fff; }
.yu-xform-table .el-form-item__label {padding-left: 10px!important;background-color: #E4F2F6; }
.yu-xform-table .el-col { border-bottom: 1px solid #E1E1E1; border-right: 1px solid #E1E1E1;}
.yu-xform-table .el-input.is-disabled .el-input__inner { color: #666;}
.yu-xform-table .el-textarea.is-disabled .el-textarea__inner { background-color: #fff; color: #666; resize: none; border: none; overflow: hidden;}
.yu-xform__arrow { 
  cursor: pointer;
  position: absolute; top: -20px; left: 50%; z-index: 999; transform: translateX(-50%); width: 51px; height: 51px; background: url('../../../assets/images/common/arrow-bg.png') no-repeat center / 100% 100%;
  text-align: center;
  img {
    margin-top: 23px;
  }
...
@media screen and (max-width: 1680px) {
  .yu-xform .yu-xform-btns { bottom: 12px; right: 12px; }
}
