  ...
  private setOptions(chartData: ILineChartData) {
    if (this.chart) {
      let data = (chartData.yData as any[]).map((item: any) => {
        return (this.roleCodes.includes((this as any).$util.getCheckedRole().roleCode) ? (this as any).$util.toFixed(parseFloat(item) / 10000) : (this as any).$util.toFixed(parseFloat(item) / 100000000))
      })
