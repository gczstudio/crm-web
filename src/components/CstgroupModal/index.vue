      ...
      <div v-loading="loading" :element-loading-text="loadingTitle">
        <el-row :gutter="16">
          <el-col :span="selectionType==='checkbox' ? 19 : 24">
            ...
            <!-- 查询结果表格 -->
            <yu-xtable v-if="dialogVisible" ref="mytable" row-number border :selection-type="selectionType" row-key="custGroupId" :height="bHeight-380" :data-url="dataUrl" @selections-change="selectionChangeFn">
...
export default {
  ...
  props: {
    ...
    isAuth: { // 默认查全部，为true时，查当前用户可导入客户的客群
      type: Boolean,
      default: false
    },
    loading: {
