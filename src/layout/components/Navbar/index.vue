  ...
  <div class="navbar">
    ...
    <div class="right-search">
      <el-input placeholder="核心客户号/企业名称/统一社会信用代码/组织机构代码" v-model="keyword" clearable @keyup.enter.native="searchFn">
        <template slot="append"><div class="search-box" @click="searchFn"><i class="el-icon-search"></i></div></template>
      </el-input>
    </div>
    <div class="right-menu">
      <div class="dv">
        <div class="right-date dv">数据日期：<span class="num">{{dataDt}}</span></div>
        <ul class="right-funcs dv">
          <li v-if="$util.getCheckedRole().roleCode === 'R100001'" ><span>在线人数</span><i class="iconfont icon-zaixianrenshu"></i><span class="num">{{userCount}}</span>人</li>
          <li>
            <el-popover
              popper-class="remind-popper"
              placement="top-start"
              width="220"
              trigger="hover"
            >
              <span slot="reference">
                <div ref="msgRef">
                  <i class="iconfont icon-gonggao"></i><span>消息（<span class="num">{{remindNum}}</span>）</span>
                </div>
              </span>
              <div class="remind-list">
                <ul>
                  <li v-for="item in remindData" :key="item.opprname" @click="remindChangeFn(item)">{{item.opprname}}<span>{{item.opprcstnum}}</span></li>
                </ul>
              </div>
            </el-popover>
          </li>
          <li @click="downFileFn"><i class="iconfont icon-xiazai"></i><span>下载（<span class="num">{{downFileNum}}</span>）</span></li>
        </ul>
