<template>
  <div>
              ...
              <yu-xtable ref="pointTable" :data-url="contrDataUrl" selection-type="checkbox" :default-load="true" border dynamic-height>
                <yu-xtable-column label="控制操作名称" min-width="120" prop="contrName">
                  <template scope="scope">
                    <a onclick="return false;" href="javascipt:void(0);" style="text-decoration: underline" @click="rowClick(scope)">{{ scope.row.contrName }}</a>
                  </template>
                </yu-xtable-column>
                <yu-xtable-column label="控制操作代码" prop="contrCode" :sortable="true" :resizable="true"></yu-xtable-column>
                <yu-xtable-column label="控制操作URL" prop="contrUrl" :show-overflow-tooltip="true" :sortable="true" :resizable="true"></yu-xtable-column>
    ...
    <yu-dialog :title="viewTitle[viewType]" :visible.sync="dialogVisible">
      <yu-xform :disabled="formDisabled" :model="pointForm" :form-type="viewType == 'DETAIL' ? 'details' : ''" ref="pointForm" label-width="120px">
...
export default {
  ...
  methods: {
    ...
    renderContent: function () {
      ...
      return createElement("span", [createElement("span", { attrs: { class: "yu-treeMenuType" + type } }, type), createElement("span", arguments[1].node.label)]);
    },
