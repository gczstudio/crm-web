<template>
  <div class="CustBank-container">
    <div class="yu-main-wrapper">
      <MainLayout>
        <template v-slot:header>
          <el-button icon="el-icon-s-operation" v-if="$checkCtr('mainOrg') || roleInfo.roleCode === 'R100001'" @click="allotFn('mainOrg', 'radio')">分配主管机构</el-button>
          <el-button icon="el-icon-s-operation" v-if="$checkCtr('assistOrg') || roleInfo.roleCode === 'R100001'" @click="allotFn('assistOrg')">分配协管机构</el-button>
          <el-button icon="el-icon-user" v-if="$checkCtr('mainMgr') || roleInfo.roleCode === 'R100001'" @click="allotFn('mainMgr', 'radio')">分配主管客户经理</el-button>
    ...
    <OrgModal :selection-type="selectionType" :is-auth-org="false" :org-codes="selectOrgIds" :visible.sync="orgVisible" @sure="orgSureFn" />
...
export default class extends Vue {
  ...
  isCustMgr() {
    return ["mainMgr", "assistMgr"].includes(this.disType);
  }

  // 确认分配机构
  orgSureFn(data: any) {
    if (!data?.length && this.disType === "mainOrg") {
      this.$message.warning("请选择一条数据！");
      return;
    }
    this.sureAllotFn("orgCodes", data);
  }
  ...
  allotFn(disType: string, selectionType: string) {
    ...
    // // 分配主管机构时
    // this.selectOrgIds = disType === 'mainOrg' ? selections[0].blgOrgNm : ''

    // 分配协管机构时
    this.selectOrgIds = disType === "assistOrg" ? (selections.length > 1 ? "" : selections[0].assistBlgOrgNo) : "";

    // // 分配主管客户经理时
    // this.selectCrmCustIds = disType === 'mainMgr' ? selections[0].blgMgrNm : ''
  ...
  sureAllotFn(type: string, data: any) {
    let params = {
      ...
      [type]: data.map((item: any) => (this.isCustMgr() ? item.userId : item.orgId)).join(","),
      manageUserId: "",
      // userIds: '',userIds
      // orgCodes: '',
