<template>
  <div class="DepositeChange-container">
      ...
      <MainLayout>
        ...
        <template v-slot:form>
          <yu-xform ref="searchForm" class="search" :model="queryFormData" :search="searchFn" form-type="search" :need-export="true">
            <yu-xform-group :column="4">
              <yu-xform-item label="开始日期"  placeholder="开始日期" name="startDate" ctype="date-picker" :picker-options="startPickerOptions" :rules="globalRules.required"></yu-xform-item>
              <yu-xform-item label="结束日期" placeholder="结束日期"  name="endDate" ctype="date-picker" :picker-options="endPickerOptions" :rules="globalRules.required"></yu-xform-item>
              ...
              <yu-xform-item label="存款变化额>=" :show-label-suffix="false" suffix-label="万元" suffix-label-width="28px" placeholder="存款变化额" ctype="input" name="chgAmt" :rules="globalRules.amountInput"></yu-xform-item>
        ...
        <template v-slot:table>
          <yu-xtable ref="refTable" :data-url="dataUrl" row-number :base-params="baseParams" :dynamic-height="true" border :default-load="false">
            <yu-xtable-column label="客户名称" prop="custNm" min-width="250" :show-overflow-tooltip="true" border sortable="custom">
            ...
            <template v-if="queryFormData.isTy == '1'">
              <yu-xtable-column label="期初余额（元）" prop="openTyBal"  min-width="160" align="right" format-money :show-overflow-tooltip="true" sortable="custom"></yu-xtable-column>
              <yu-xtable-column label="期末余额（元）" prop="closeTyBal"  min-width="160" align="right" format-money :show-overflow-tooltip="true" sortable="custom"></yu-xtable-column>
              <yu-xtable-column label="存款变化额（元）" prop="chgTyAmt"  min-width="160" align="right" format-money :show-overflow-tooltip="true" sortable="custom"></yu-xtable-column>
            </template>
            <template v-else>
              <yu-xtable-column label="期初余额（元）" prop="openBal"  min-width="160" align="right" format-money :show-overflow-tooltip="true" sortable="custom"></yu-xtable-column>
              <yu-xtable-column label="期末余额（元）" prop="closeBal"  min-width="160" align="right" format-money :show-overflow-tooltip="true" sortable="custom"></yu-xtable-column>
              <yu-xtable-column label="存款变化额（元）" prop="chgAmt"  min-width="160" align="right" format-money :show-overflow-tooltip="true" sortable="custom"></yu-xtable-column>
            </template>
...
export default class extends Vue {
  ...
  private dataUrl = backend.custService + '/api/custdepanalybycst/depchglist'
  ...
  typeChangeFn(val:string) {
    this.dataUrl = backend.custService + (val === '1' ? '/api/custdepanalybycst/depchglist' : '/api/custdepanaly/depchglist') 
  ...
  exportFn (showTipModal?:boolean) {
    ...
    var apiParams = {
      url: backend.custService + (this.queryFormData.type === '1' ? '/api/custdepanalybycst/exportdepchglist' : '/api/custdepanaly/exportdepchglist'),
      params: searchQuery,
