...
export default {
  ...
  methods: {
    ...
    serachFn (data, type) {
      ...
      let conditionAttrs = [], results = [], tagDateFlag = null;
   ...
   /**
    ...
    validateForm () {
      ...
      // 搜索条件表单校验
      let bool = this.conditionList.map((item, index) => {
        let validate = false;
    ...
    getScopeOptions (data) {
      ...
      if (colType == 'VARCHAR2') { // 文本框
        selection.options = textOptions;
      } else if (colType == 'NUMBER' || colType == 'DECIMAL' || colType == 'INTEGER') { // 数字框
        selection.options =numDataOptions;
      } else if (colType == 'DATE' || colType == 'TIMESTMP') { // 日期框
        selection.options = numDataOptions;
      }
    ...
    exportFn (showTipModal) {
      ...
      queryField = this.conditionList.map(item => {
        const { name, prop, ssColOp, selection: { options } } = item
