<template>
  <div class="drag-table">
    <yu-xtable ref="refTable" :key="key" v-if="dropCol.length" :data-url="dataUrl" border row-key="id" align="left" :default-load="false" selection-type="checkbox" @header-dragend="headerDragendFn">
      <template v-for="item in dropCol">
        ...
        <yu-xtable-column
          ...
          align="right"
          :label="item.label"
          :render-header="renderHeaderFn"
          :show-overflow-tooltip="true"
        ></yu-xtable-column>
        <!-- 数字类型 -->
        <yu-xtable-column
          v-else-if="isNum(item)"
          :key="item.prop"
          :min-width="item.width"
          :prop="item.prop"
          align="right"
      ...
      return (
        <div class={`header-wrapper header_${$index}`}>
          ...
          <span class="caret-wrapper" on-click={($event) => this.handleSortClick($event, column)}>
            <i class="sort-caret ascending" on-click={($event) => this.handleSortClick($event, column, "asc")}></i>
            <i class="sort-caret descending" on-click={($event) => this.handleSortClick($event, column, "desc")}></i>
          </span>
