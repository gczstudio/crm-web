    ...
    <div v-if="dialogVisible">
      ...
      <div class="form">
        <yu-xform ref="form1" :model="formData1" label-width="123px" :disabled="formData1.mntMode === '1'">
          <yu-xform-group>
            <yu-xform-item label="标签类型" placeholder="标签类型" ctype="yufp-tree-select" name="parentTagNo" :tree-options="tagTreeOptions" :rules="formData1.mntMode === '1' ? null : globalRules.required" @select-fn="tagSelectFn" :disabled="formData1.mntMode === '1'"></yu-xform-item>
            ...
            <yu-xform-item label="管理角色" placeholder="管理角色" ctype="select" name="mgrRole" :options="roleOptions" :rules="formData1.mntMode === '1' ? null : globalRules.required" multiple filterable></yu-xform-item>
        ...
        <div class="tag-section"><span v-if="formData1.mntMode != '1'" style="color: #f56c6c;margin-right: 4px;">*</span><span>标签值</span> <img v-if="formData1.mntMode != '1'" :src="require('@/assets/images/common/icon-plus.png')" alt="" @click="addTagValueFn"></div>
        <yu-xform ref="form2" class="tag-value-form" :model="formData2" label-width="90px" v-loading="loading">
          <div class="form-section" v-for="(item, index) in tagValues" :key="item.id">
            <yu-xform-group :column="3">
              <yu-xform-item label="标签值" placeholder="标签值" ctype="input" :name="`tagItemCode_${index}`" :disabled="type ==='edit' && !item.isAdd" :rules="formData1.mntMode === '1' ? null : globalRules.requiredInput" @change="tagValueChange($event, index, 'tagItemCode')"></yu-xform-item>
              <yu-xform-item label="文字说明" placeholder="文字说明" ctype="input" :name="`itemCodeNm_${index}`" :disabled="formData1.mntMode == '1'" :rules="formData1.mntMode === '1' ? null : globalRules.required500" @change="tagValueChange($event, index, 'itemCodeNm')"></yu-xform-item>
...
export default class extends Vue {
  ...
  sureFn() {
    ...
    if((this.formData1 as any).mntMode != '1') {
      var validate1 = false, validate2 = false;
  ...
  getNamesByCode () {
    return this.roleOptions.filter((item: any) => {
