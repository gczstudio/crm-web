            ...
            <yu-xtable-column label="余额（元）" prop="depBal" width="200" fixed="right" align="right" class-name="dep-bal-column" sortable="custom">
              <template slot-scope="scope">
                <div :class="['num', 'dep-bal', scope.$index === 0 ? 'first' : '']">
                  <div class="dep-tooltip" v-if="refTable.selections[0] && refTable.selections[0].acctSubNo === scope.row.acctSubNo && realAmount[scope.row.acctSubNo]">
                    ¥ {{ realAmount[scope.row.acctSubNo] ? $util.formatMoney(realAmount[scope.row.acctSubNo].balance) : "" }}
                  </div>
...
export default class extends Vue {
  ...
  transDetailFn() {
    ...
    this.$router.push({
      path: "/custInfo/custRunAccount/" + selections[0].acctSubNo,
      query: { acctSubNo: selections[0].acctSubNo, crmCustId: this.crmCustId, title: "交易明细-" + selections[0].acctSubNo },
    });
  }
