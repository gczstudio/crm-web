<template>
  <div class="TrendChart-container">
    <div class="chart-title">交易金额趋势图<small-tab class="fr" type="top" v-model="curTab" :data="['日','月']" @change="tabChange"></small-tab></div>
    <div id="inOutTrendChart" style="height: 340px"></div>
  </div>
</template>
...
export default class extends mixins(ResizeMixin){
  ...
  renderChart(data: []) {
    let chart = echarts.init((document.getElementById('inOutTrendChart') as HTMLElement))
    chart.setOption({
      ...
      series: [
        {
          ...
          data: data.map((item: any) => item.inflowAmt || 0),
        ...
        },
        {
          ...
          data: data.map((item: any) => item.outflowAmt || 0),
          animationDuration: 2800,
          animationEasing: 'cubicInOut'
        },
        {
          ...
          lineStyle: {
            width: 3,
            shadowColor: 'rgba(248, 222, 99, 0.3)',
            shadowBlur: 10,
            shadowOffsetY: 8
          },
          data: data.map((item: any) => item.inflowTotalAmt || 0),
          animationDuration: 2800,
          animationEasing: 'cubicInOut'
        }
      ]
