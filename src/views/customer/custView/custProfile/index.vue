<template>
  <div class="custProfile-container clearfix">
    <div class="custProfile-left fl">
      <div class="custProfile-left-c">
        ...
        <div class="custProfile-left-b">
          <div class="base-info">
            <p>
              <span class="label">核心客户号：</span><span class="num">{{ baseInfo.custId }}</span>
            </p>
            <p><span class="label">法定代表人：</span>{{ baseInfo.legalName }}</p>
          ...
          <div class="address">
            <div class="address-title">
              ...
              <div>
                地址：<span v-if="baseInfo.contAddr">{{ baseInfo.contAddr }}</span> <span v-else class="c3">暂无地址</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="cust-credit">
        <div class="cust-credit__title"><i class="iconfont icon-shouxinjine1 vam f3"></i><span class="vam f3">客户授信金额及使用情况</span></div>
        <div class="cust-credit__content">
          <div class="credit-card clearfix">
            <div class="credit-card-l fl">
              <p class="subtitle">授信总额</p>
              <p class="total">
                <span class="num">{{ $util.formatMoney(creditInfo.crdtAmt / 10000) }}</span
                ><span class="unit">万元</span>
              </p>
              <p class="subtitle" style="margin-top: 8px">已使用授信额度</p>
              <p class="total">
                <span class="num">{{ $util.formatMoney(creditInfo.usedCrdtAmt / 10000) }}</span
                ><span class="unit">万元</span>
              </p>
            </div>
          ...
          <div class="credit-card clearfix">
            <div class="credit-card-l fl">
              ...
              <p class="total">
                <span class="num">{{ $util.formatMoney(creditInfo.crdtExpoAmt / 10000) }}</span
                ><span class="unit">万元</span>
              </p>
              <p class="subtitle" style="margin-top: 8px">已使用敞口额度</p>
              <p class="total">
                <span class="num">{{ $util.formatMoney(creditInfo.uCrdtExpoAmt / 10000) }}</span
                ><span class="unit">万元</span>
              </p>
            </div>
            <div class="credit-card-r">
    ...
    <div class="custProfile-right fl">
      <el-row :gutter="isMaxScreen ? 16 : 12">
        <el-col :span="8" v-for="(item, index) in cardData" :key="item.chartId" class="mb16">
        ...
        <el-row :gutter="isMaxScreen ? 16 : 12">
          ...
          <el-col :span="8">
            <div class="seciton-right">
              <div class="rate-card">
                <img :src="require('@/assets/images/customer/cun.png')" alt="" />
              ...
              <div class="rate-card">
                <img :src="require('@/assets/images/customer/dai.png')" alt="" />
                <div class="rate-card__content">
      ...
      <div class="cust-account mt16">
        <div class="cust-account__title">
          <i class="iconfont icon-kehuxinxi1 vam f3"></i
          ><span class="vam f3"
            >客户在我行的账户信息（<span class="num">{{ accountTotal }}</span
            >）</span
          >
        ...
        <div class="cust-account__content">
          <el-carousel arrow="always" :autoplay="false" height="218px" indicator-position="none" v-if="accountList.length">
                ...
                <el-row :gutter="16" style="padding: 10px">
                  <el-col :span="12" v-if="accountList[index * 2]">
                    <div class="account-card">
                      ...
                      <p class="account-total f5">
                        余额：￥<span class="num">{{ $util.formatMoney(accountList[index * 2].depBal) }}</span
                        >元
                      </p>
                  ...
                  <el-col :span="12" v-if="accountList[index * 2 + 1]">
                    <div class="account-card">
                      ...
                      <p class="account-total f5">
                        余额：￥<span class="num">{{ $util.formatMoney(accountList[index * 2 + 1].depBal) }}</span
                        >元
                      </p>
                ...
                <el-row :gutter="16" style="padding: 10px">
                  <el-col :span="8" v-if="accountList[index * 3]">
                    <div class="account-card">
                      ...
                      <p class="account-total f5">
                        余额：￥<span class="num">{{ $util.formatMoney(accountList[index * 3].depBal) }}</span
                        >元
                      </p>
                  ...
                  <el-col :span="8" v-if="accountList[index * 3 + 1]">
                    <div class="account-card">
                      ...
                      <p class="account-total f5">
                        余额：￥<span class="num">{{ $util.formatMoney(accountList[index * 3 + 1].depBal) }}</span
                        >元
                      </p>
                  ...
                  <el-col :span="8" v-if="accountList[index * 3 + 2]">
                    <div class="account-card">
                      ...
                      <p class="account-total f5">
                        余额：￥<span class="num">{{ $util.formatMoney(accountList[index * 3 + 2].depBal) }}</span
                        >元
                      </p>
...
export default class extends Vue {
  ...
  custRunAccountFn(acctNo: string) {
    this.$router.push({
      path: "/custInfo/custRunAccount/" + acctNo,
      query: {
        acctNo,
        crmCustId: this.crmCustId,
        title: "交易流水-" + acctNo,
  ...
  renderPieChart(id: string, color: string[], keyObj: any) {
    let chart = echarts.init(document.getElementById(id) as HTMLElement);
    chart.setOption({
      color: color,
      tooltip: {
        trigger: "item",
      ...
      .cust-account__content {
        ...
        .account-card {
          ...
          .account-status {
            ...
            padding: 4px 8px;
            color: #0fcb51;
            background: rgba(15, 203, 81, 0.1);
            &.error {
              color: #f14922;
              background: rgba(241, 73, 34, 0.1);
              border-radius: 2px;
            }
          }
          .account-total {
