  ...
  <div class="TeamList-component">
    <MainLayout is-tab>
      <template v-slot:header>
        <el-button icon="el-icon-download" v-permission="'exportTeam'" @click="exportFn">导出</el-button>
      </template>
      ...
      <template v-slot:table>
        <yu-xtable ref="refTable" :data-url="dataUrl" row-number :base-params="baseParams" selection-type="checkbox" :dynamic-height="true" border>
          <yu-xtable-column label="团队名称" prop="teamNm" width="120" fixed="left" :show-overflow-tooltip="true" sortable="custom"></yu-xtable-column>
          <yu-xtable-column label="团队长姓名" prop="mebNm" width="120" :show-overflow-tooltip="true" fixed="left" sortable="custom"></yu-xtable-column>
          <yu-xtable-column label="团队成员数" prop="mebNumber" width="130" is-num :show-overflow-tooltip="true" sortable="custom"></yu-xtable-column>
          ...
          <yu-xtable-column label="操作" width="140" fixed="right" align="center" v-permission="'viewTeamCust'">
    ...
    <content-modal :visible.sync="contentVisible">
      <template slot-scope="scope">
        <team-cust-list :instance="scope" :row="currentRow" />
      </template>
    </content-modal>
  </div>
</template>
