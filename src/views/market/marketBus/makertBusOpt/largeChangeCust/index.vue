<template>
  <div class="largeChangeCust-component">
    <MainLayout :title="title">
      <template v-slot:header>
        <el-button icon="el-icon-download" v-permission="'exportLarge'" @click="exportFn">导出</el-button>
      ...
      <template v-slot:table>
        <div style="padding-top: 16px">
          <yu-xtable ref="refTable" :data-url="dataUrl" row-number selection-type="checkbox" :dynamic-height="true" border :default-load="false">
            ...
            <yu-xtable-column label="上日累计交易额（元）" prop="amount" width="200" align="right" format-money :show-overflow-tooltip="true" sortable="custom"></yu-xtable-column>
...
export default class extends Vue {
  ...
  private dataUrl = backend.mgrMkt + "/api/mkcust/querylong";
  ...
  mounted() {
    this.getTableData();
    this.$checkCtr("exportLarge") && this.$exportQueue.addQueue(this.$route.path, this.exportFn, true);
  }
  ...
  exportFn(showTipModal?: boolean) {
    ...
    var apiParams = {
      url: backend.mgrMkt + "/api/mkcust/exportquerylong",
      params: searchQuery,
